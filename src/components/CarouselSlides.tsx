  ...
  const media_data = {
    imdbId:
      carouselSlides.length > 0
        ? carouselSlides[Math.abs(count)]["imdbId"]
        : "",
    title:
      carouselSlides.length > 0 ? carouselSlides[Math.abs(count)]["title"] : "",
    poster: `https://image.tmdb.org/t/p/original${
      carouselSlides.length > 0 ? carouselSlides[Math.abs(count)]["poster"] : ""
    }`,
    plot:
      carouselSlides.length > 0 ? carouselSlides[Math.abs(count)]["plot"] : "",
    runtime:
      carouselSlides.length > 0
        ? carouselSlides[Math.abs(count)]["runtime"]
        : "",
    year:
      carouselSlides.length > 0 ? carouselSlides[Math.abs(count)]["year"] : "",
    genre:
      carouselSlides.length > 0 ? carouselSlides[Math.abs(count)]["genre"] : "",
    media: "movie",
  };
    ...
    if (carouselData) {
      const arr = carouselData.map((_: any, idx: number) => {
        return {
          ...
          poster: `https://image.tmdb.org/t/p/original${
            carouselData?.[Math.abs(idx)]["Poster"]
          }`,
    ...
    <>
      ...
      <div className="z-10 flex justify-between w-full px-2">
        <button
          ...
          onClick={() => setCount((p) => (p === 0 ? (p + 6) % 7 : (p - 1) % 7))}
        >
          <ArrowBackIosIcon />
        </button>
        <button
          className="text-white"
          onClick={() => setCount((p) => (p > 6 ? 1 : (p + 1) % 7))}
        >
          <ArrowForwardIosIcon />
        </button>
      </div>
      {carouselSlides.map((media_data_carousel: CarouselType, i: number) => {
        return (
          <div
            key={i}
            className={`absolute z-0 flex w-4/5 h-full mx-5 ${
            ...
            style={{
              backgroundImage: `url(${media_data_carousel.poster})`,
              backgroundRepeat: "no-repeat",
              backgroundSize: "cover",
              transform: `translateX(${(i - count) * 100}%)`,
            }}
          >
            <div className="absolute w-4/5 h-full p-1 bg-gradient-to-r from-black to-transparent">
              <div className="flex">
                <div className="flex flex-col p-6 md:w-4/5">
                  <p className="text-2xl font-bold text-white md:text-5xl">
                    {media_data_carousel.title}
                  </p>
                  <p className="hidden p-1 text-lg text-white lg:block">
                    {media_data_carousel.plot}
                  </p>

                  <div className="flex-col justify-start hidden mt-4 md:flex md:items-start lg:flex-row">
                    <p className="flex items-center px-1 text-lg font-semibold text-white">
                      <AlbumIcon /> &nbsp;- {media_data_carousel.genre}
                    </p>
                    <ContentSeparator />
                    <p className="flex items-center px-1 text-lg font-semibold text-white">
                      <CalendarMonthIcon /> &nbsp;- {media_data_carousel.year}
                    </p>
                    <ContentSeparator />
                    <p className="flex items-center px-1 text-lg font-semibold text-white">
                      <AccessTimeIcon /> &nbsp;-{" "}
                      {getHumanizedTimeFromMinutes(
                        parseInt(media_data_carousel.runtime)
                      )}
                    </p>
                  </div>
