...
const drawerData = [
  ...
  { title: "Series", icon: <TheaterComedyIcon />, link: "/shows" },
  {
    title: "Watchlist",
...
const Header: React.FC<Record<string, never>> = () => {
  ...
  useEffect(() => {
    ...
    ctx?.setEmail(JSON.parse(localStorage.getItem("email") as string) ?? null);
  }, []);
      ...
      <div className="w-full">
        <div className={`flex justify-between w-full p-4 bg-gray-100`}>
          <Link to="/" className={`${ctx?.auth && "hidden"} lg:flex`}>
            <img
              src={require("../assets/bingenow.png")}
              alt="Bingenow logo"
          ...
          {ctx?.auth && (
            <>
              <div className="flex items-center px-2 lg:hidden">
                ...
                >
                  <svg
                    className="w-6 h-6 text-gray-500 hover:text-purple-800"
                    x-show="!showMenu"
                    fill="none"
                    strokeLinecap="round"
